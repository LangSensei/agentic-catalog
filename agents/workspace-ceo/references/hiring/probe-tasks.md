...
```sh
echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) | PROBE | <fqn> | <pass|fail> | <one-line summary>" >> .ceo/decisions.log
```

If pass, append to `.ceo/hires.md`:
...
If fail, write a one-line note in `.ceo/hires.md` AND DON'T USE THIS AGENT for real work. Try the next candidate in the hiring decision tree.
