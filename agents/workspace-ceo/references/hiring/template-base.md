...
scope: local                      # convention: 'local' for CEO-created agents
description: "<one-line: what this agent does>"
version: 1.0.0                    # bump on edits
---
```

**Do NOT include `dependencies.skills: [emploke-cli]`.** Subagents don't orchestrate. The CEO is the only one with that skill.
...
5. If the probe passes, update `.ceo/hires.md` with the new version's outcome.
...
Append to `.ceo/decisions.log`: `RETIRE | local/$NAME | <reason>`.
