...
- `.ceo/` directory and everything in it
...
When you start up and detect `.ceo/` exists:
...
cat .ceo/identity.md
cat .ceo/strategy.md
ls -t .ceo/letters/ | head -1 | xargs -I{} cat .ceo/letters/{}
tail -20 .ceo/decisions.log

# 3. Reconcile in-flight tasks vs your tracked state.
emploke task list --status running --json > /tmp/server-running.json
# Compare against .ceo/active-missions/*/tasks.json
# Look for:
...
echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) | RESUME | Recovered after $((NOW - LAST_TICK_EPOCH)) sec downtime; processed N catchup completions; M in-flight tasks reconciled." >> .ceo/decisions.log
...
- Another CEO instance ran (this is bad — only one CEO per workspace; surface this to user).
