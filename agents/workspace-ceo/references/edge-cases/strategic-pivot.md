...
```sh
DATE=$(date -u +%Y-%m-%d)
mkdir -p .ceo/strategy-history/$DATE
cp .ceo/strategy.md .ceo/strategy-history/$DATE/
cp .ceo/org-chart.md .ceo/strategy-history/$DATE/
cp -r .ceo/active-missions .ceo/strategy-history/$DATE/
echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) | PIVOT_SNAPSHOT | Captured pre-pivot state" >> .ceo/decisions.log
```
...
- **Not aligned** → archive
  ...
  echo "Outcome: archived due to strategic pivot on $DATE. Pre-pivot state preserved in strategy-history/$DATE." > .ceo/active-missions/<id>/outcome.md
  mv .ceo/active-missions/<id> .ceo/archived-missions/
  ```
...
In `.ceo/reports/<date>-pivot.md`, write a longer narrative for the audit log.
...
```sh
cat > .ceo/letters/$(date +%Y-%m-%d)-pivot.md <<EOF
