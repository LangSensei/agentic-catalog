...
Cross-operation analysis and optimization of existing agents and skills in [emploke-marketplace](https://github.com/LangSensei/emploke-marketplace).

## Boundary

**In scope:**
- Reading a target agent's operation history (findings, reports, knowledge files)
...
- Extracting reusable knowledge from operation history into new skills
...
- Modifying agents/skills based on user feature requests without operation history (that's `agent-forge`)
...
2. **Read operation history** — for each operation in scope, read the artifacts the runtime persisted:
...
3. **Pattern identification** — look for:
   ...
   - **Recurring failures** — same error across operations → needs a guard in the playbook or a fix
   ...
   - **Stale references** — entries that haven't been used or relevant in recent operations → candidate for removal
...
- List reference entries with last-used operation and reference count
...
2. One PR per distill operation — title format: `distill({target-agent}): {summary}`
...
Report should include: operations analyzed, patterns identified, changes made (with evidence), reference entries pruned, and recommendations for future distill cycles.
